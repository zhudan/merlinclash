       ...
       if [ -n "$pid_clash" ]; then
              ...
              if [ "$?" == "0" ];then
                     ...
                     if [ -z "$(cat /tmp/ChinaIP.list)" ];then 
                            ...
                            wget --no-check-certificate --timeout=10 --tries=3 https://raw.githubusercontents.com/fernvenue/chn-cidr-list/master/ipv4.yaml -O /tmp/ChinaIP.list #>/dev/null 2>&1
                     fi
              ...
              echo_date "从raw.githubusercontents.com端下载" >> $LOG_FILE
              wget --no-check-certificate --timeout=10 --tries=3 https://raw.githubusercontents.com/fernvenue/chn-cidr-list/master/ipv4.yaml -O /tmp/ChinaIP.list #>/dev/null 2>&1
       fi
       ...
       if [ -n "$pid_clash" ]; then
              ...
              if [ "$?" == "0" ];then
                     ...
                     if [ -z "$(cat /tmp/ChinaIPv6.list)" ];then 
                            echo_date "获取大陆IP白名单文件失败！使用CDN地址下载" >> $LOG_FILE
                            wget --no-check-certificate --timeout=10 --tries=3 https://raw.githubusercontents.com/fernvenue/chn-cidr-list/master/ipv6.yaml -O /tmp/ChinaIPv6.list #>/dev/null 2>&1
                     fi
