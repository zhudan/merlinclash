...
start_online_update_hnd(){
	...
	if [ "$customrule" == "0" ]; then
		...
		_name="CUS_"
		subscription_type="6"
		links="http://127.0.0.1:25500/sub?target=$clashtarget&new_name=true&url=$merlinc_link&insert=false&config=customconfig%2F${acl4ssrsel}.ini&include=$include&exclude=$exclude&append_type=$appendtype&emoji=$emoji&udp=$udp&fdn=$fdn&sort=$sort&scv=$scv&tfo=$tfo"
	fi
	if [ "$URLflag" == "1" ]; then
		_name="MY_"
...
start_regular_update_hnd(){
	...
	if [ "$subscription_type" == "4" ]; then
		...
		links="http://127.0.0.1:25500/sub?target=$clashtarget&new_name=true&url=$merlinc_link&insert=false&config=ruleconfig%2F${acltype_tmp}.ini&include=$include&exclude=$exclude&append_type=$appendtype&emoji=$emoji&udp=$udp&fdn=$fdn&sort=$sort&scv=$scv&tfo=$tfo"
	elif [ "$subscription_type" == "6" ]; then
		_name="CUS_"
		links="http://127.0.0.1:25500/sub?target=$clashtarget&new_name=true&url=$merlinc_link&insert=false&config=customconfig%2F${acltype_tmp}.ini&include=$include&exclude=$exclude&append_type=$appendtype&emoji=$emoji&udp=$udp&fdn=$fdn&sort=$sort&scv=$scv&tfo=$tfo"
	else
		_name="MY_"
		links="http://127.0.0.1:25500/sub?target=$clashtarget&new_name=true&url=$merlinc_link&insert=false&config=${urlinilink}&include=$include&exclude=$exclude&append_type=$appendtype&emoji=$emoji&udp=$udp&fdn=$fdn&sort=$sort&scv=$scv&tfo=$tfo"
		dbus set merlinclash_customrule_cbox="0"
	...
	echo_date "订阅地址是：$merlinc_link" >> $LOG_FILE
		...
		if [ "$URLflag" == "0" ]; then
			...
			if [ "$acl_flag" == "ACL4SSR" ] && [ "$mcc" == "0" ]; then
				...
				links="${addr}sub?target=$clashtarget&new_name=true&url=$merlinc_link&insert=false&config=https%3a%2f%2fraw.githubusercontents.com%2FACL4SSR%2FACL4SSR%2Fmaster%2FClash%2Fconfig%2f${acl4ssrsel}.ini&include=$include&exclude=$exclude&append_type=$appendtype&emoji=$emoji&udp=$udp&fdn=$fdn&sort=$sort&scv=$scv&tfo=$tfo"
