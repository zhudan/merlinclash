...
check_rule() {	
	...
	if [ "$yamlselchange" == "1" ]; then
		...
		/bin/sh /koolshare/scripts/clash_saveacls.sh use use 
	fi
	# acl_nu 获取已存数据序号
			...
			for acl in $acl_nu; do
				...
				if [ "$type" == "IP-CIDR" ]; then
					...
					if [ "$protocol" != "none" ]; then
						sed "$rules_line a \ \ -\ $type,$content,$lianjie,$protocol" -i $yamlpath
					else
...
set_sys() {
	...
	if [ -z $(which jitterentropy-rngd) -a -f "/koolshare/bin/haveged_c" ]; then
		echo_date "启动haveged，为系统提供更多的可用熵！"
...
creat_ipset() {
  ...
  [ -n "$IFIP_DNS1" ] && ISP_DNS_a="$ISP_DNS1" || ISP_DNS_a=""
	...
	#ipset：kp_full_port
	cat /koolshare/merlinclash/koolproxy/data/rules/koolproxy.txt /koolshare/merlinclash/koolproxy/data/rules/daily.txt /koolshare/merlinclash/koolproxy/data/rules/user.txt | grep -Eo "(.\w+\:[1-9][0-9]{1,4})/" | grep -Eo "([0-9]{1,5})" | sort -un | sed -e '$a\80' -e '$a\443' | sed -e "s/^/-A kp_full_port &/g" -e "1 i\-N kp_full_port bitmap:port range 0-65535 " | ipset -R -!
	...
	#done
	cat /koolshare/merlinclash/koolproxy/data/rules/koolproxy.txt /koolshare/merlinclash/koolproxy/data/rules/daily.txt /koolshare/merlinclash/koolproxy/data/rules/user.txt | grep -Eo "(.\w+\:[1-9][0-9]{1,4})/" | grep -Eo "([0-9]{1,5})" | sort -un | sed -e '$a\80' | sed -e "s/^/-A kp_port_http &/g" -e "1 i\-N kp_port_http bitmap:port range 0-65535 " | ipset -R -!
	cat /koolshare/merlinclash/koolproxy/data/rules/koolproxy.txt /koolshare/merlinclash/koolproxy/data/rules/daily.txt /koolshare/merlinclash/koolproxy/data/rules/user.txt | grep -Eo "(.\w+\:[1-9][0-9]{1,4})/" | grep -Eo "([0-9]{1,5})" | sort -un | sed -e '$a\80' | sed -e "s/^/-A kp_port_https &/g" -e "1 i\-N kp_port_https bitmap:port range 0-65535 " | ipset -R -!
...
pre_netflix_nslookup(){
	...
	if [ "$netflix_flag" == "1" ]; then
		NETFLIX_DOMAINS_LIST="/koolshare/res/Netflix_Domains.list"
		NETFLIX_DOMAINS_CUSTOM_LIST="/koolshare/res/Netflix_Domains_Custom.list"
		DISNEYPLUS_DOMAINS_LIST="/koolshare/res/DisneyPlus_Domains.list"
		if [ "$1" != "back" ]; then
...
creat_router_ipset(){
	...
	rm -rf /koolshare/merlinclash/conf/kpipset.conf >/dev/null 2>&1
	rm -rf /jffs/configs/dnsmasq.d/kpipset.conf >/dev/null 2>&1
	rm -rf /koolshare/merlinclash/conf/kpipsetarround.conf >/dev/null 2>&1
	rm -rf /jffs/configs/dnsmasq.d/kpipsetarround.conf >/dev/null 2>&1
	...
	sh /koolshare/scripts/clash_ipsetproxychange.sh ip ip
	echo_date " " >> $LOG_FILE
	echo_date "开始创建强制绕行Clash的ipset规则集" >> $LOG_FILE
	sh /koolshare/scripts/clash_ipsetproxyarroundchange.sh ip ip
	echo_date " " >> $LOG_FILE
	echo_date "开始创建KP自定义过滤规则集" >> $LOG_FILE
	sh /koolshare/scripts/clash_ipsetproxychange.sh kp kp
	echo_date " " >> $LOG_FILE
	...
	#	sh /koolshare/scripts/clash_routerrule.sh apply apply
	#fi
}
load_nat() {
  # 开启socks端口
...
apply_nat_rules3() {
	...
	if [ "$tproxymode" == "closed" ] || [ "$tproxymode" == "udp" ]; then
		...
		if [ "$dnsgoclash" == "1" ]; then
			...
			iptables -t mangle -A merlinclash_OUTPUT -s $lan_ipaddr -j RETURN
			iptables -t mangle -A merlinclash_OUTPUT -p tcp -m set --match-set direct_list dst -j RETURN
            iptables -t mangle -A merlinclash_OUTPUT -m mark --mark $mcrm -j RETURN
			if [ "$cirswitch" == "1" ]; then	
				iptables -t mangle -A merlinclash_OUTPUT -p tcp -m set ! --match-set china_ip_route dst -j MARK --set-mark 0x2333
			else
				iptables -t mangle -A merlinclash_OUTPUT -p tcp -j MARK --set-mark 0x2333
			fi
			iptables -t mangle -I OUTPUT -j merlinclash_OUTPUT
			iptables -t mangle -I merlinclash_divert -p tcp -s $wan_ipaddr -j TPROXY --on-ip 127.0.0.1 --on-port $tproxy_port --tproxy-mark 0x2333
			if [ "$ipv6_flag" == "1" ]; then
				ip6tables -t mangle -N merlinclash_OUTPUT
				ip6tables -t mangle -A merlinclash_OUTPUT -p tcp -m set --match-set direct_list6 dst -j RETURN
                ip6tables -t mangle -A merlinclash_OUTPUT -m mark --mark $mcrm -j RETURN
				if [ "$cirswitch" == "1" ]; then	
				    ip6tables -t mangle -A merlinclash_OUTPUT -p tcp -m set ! --match-set china_ip_route6 dst -j MARK --set-mark 0x2333
				else
				    ip6tables -t mangle -A merlinclash_OUTPUT -p tcp -j MARK --set-mark 0x2333
			    fi
				ip6tables -t mangle -I OUTPUT -j merlinclash_OUTPUT
				#取到wan口ipv6地址
				router_wan0_ip6=$(ip -6 addr show dev ppp0 | sed -n '/inet/{s!.*inet6* !!;s!/.*!!p}' | sed 's/peer.*//' | grep -v '^fe80')
				ip6tables -t mangle -I merlinclash_divert -p tcp -s $router_wan0_ip6 -j TPROXY --on-ip ::1 --on-port $tproxy_port --tproxy-mark 0x2333
			fi
		fi
		...
		if [ "$dnsgoclash" == "1" ]; then
			...
			iptables -t nat -N merlinclash_OUTPUT
			iptables -t nat -A merlinclash_OUTPUT -p tcp -m set --match-set direct_list dst -j ACCEPT
            iptables -t nat -A merlinclash_OUTPUT -m mark --mark $mcrm -j ACCEPT
			iptables -t nat -A merlinclash_OUTPUT -p udp --dport 53 -j REDIRECT --to-port 53
			iptables -t nat -I OUTPUT -j merlinclash_OUTPUT
			#iptables -t mangle -I OUTPUT -p tcp -m set --match-set router dst -j MARK --set-mark 0x2333
			iptables -t mangle -N merlinclash_OUTPUT
...
restart_dnsmasq() {
	...
	#				echo "dhcp-option-force=br1,6,"${nameserver} >> /jffs/configs/dnsmasq.d/dns_custom.conf
	#			    echo "dhcp-option-force=br2,6,"${nameserver} >> /jffs/configs/dnsmasq.d/dns_custom.conf
	#				fi
	#		done
	#	fi 
	#else
...
start_clash(){
	...
	if [ "$cdc" == "1" ]; then
		...
		echo_date "检查日志延迟时间:$delaytime秒" >> $LOG_FILE
		sleep $delaytime
	else
		if [ "$merlinclash_flag" == "HND" ]; then
			echo_date "默认检查日志延迟时间:2秒" >> $LOG_FILE
			sleep 2s
		else
			echo_date "默认检查日志延迟时间:5秒" >> $LOG_FILE
			sleep 5s
		fi
	fi
	...
	if [ ! -z "$(pidof clash)" -a ! -z "$(netstat -anp | grep clash)" -a ! -n "$(grep "Parse config error" /tmp/clash_run.log)" ] ; then
		...
		if [ "$d2s" == "1" ]; then
			...
			/koolshare/bin/mc_dns2socks 127.0.0.1:${socksport} ${d2s_dnsnp} 127.0.0.1:${d2s_lp} >/dev/null  2>&1 &
			if [ ! -z "$(pidof mc_dns2socks)" ]; then
				echo_date "Dns2Socks 进程启动成功！(PID: $(pidof mc_dns2socks))"
				echo_date "启动dns2socks看门狗"
				cru a d2s_watchdog "*/1 * * * * /bin/sh /koolshare/scripts/clash_d2swatchdog.sh"
			else
...
check_dnsplan(){
	...
	rh)
		...
		if [ "$d2s" == "0" ]; then
			...
			cat /koolshare/merlinclash/yaml_dns/redirhost.yaml > /tmp/rh_tmp.yaml
...
apply_mc() {
	...
	echo_date ""
	echo_date ----------------- 预解析NETFLIX和DISNEY+ 开始-------------------- >> $LOG_FILE
	pre_netflix_nslookup
